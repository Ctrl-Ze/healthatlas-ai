        ...
        for t in tests:
            ...
            try:
                if low_bound is not None and value < low_bound:
                    ...
                    low.append(analyte)
                    interpretation = (
                        f"{analyte} is below normal ({value} < {low_bound})."
                    )
                    warnings.append(
                        f"{analyte}: {value}{t.get('unit', '')} is below normal ({low_bound}-{high_bound})"
                    )
                elif high_bound is not None and value > high_bound:
                    status = "high"
                    high.append(analyte)
                    interpretation = (
                        f"{analyte} is above normal ({value} > {high_bound})."
                    )
                    warnings.append(
                        f"{analyte}: {value}{t.get('unit', '')} is above normal ({low_bound}-{high_bound})"
                    )
                else:
                    status = "normal"
